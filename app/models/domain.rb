class Domain < ApplicationRecord
  ...
  scope :redirects, -> { where(hosting_type: 'std_fwd') }
  ...
  def self.pagespeedable
    active.hosted.where.not(id: PagespeedTest.where("timestamp > ?", 7.days.ago).pluck(:domain_id))
  end

  def self.statable
    active.hosted.where.not(id: DomainStat.where("timestamp > ?", 7.days.ago).pluck(:domain_id))
  end
